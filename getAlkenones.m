function Output=getAlkenones(varargin)

    % getAlkenones calculates the peak areas, UK37, UK38_et, and UK38_me indices. We 
    ...
    % RT - Estimated retention times of the peaks of interest in each file as a cell array. The order of entry is:
    %           RT={C37:3;C37:2;C38:3et;C38:3me;C38:2et;C38:2me};
    %           or, if you have an internal std:
    %           RT={IS;C37:3;C37:2;C38:3et;C38:3me;C38:2et;C38:2me};
    %           You may input 1-2 internal standards, as long as they elute before
    %           the alkenones.
    %           Note: You can also change the RT in the code to match your GC
    %           values by changing the variable 'RT_Sample' below. 
    ...
    % UK37 - UK37 indices corresponding to each file
    % UK38_et - UK38_et indices corresponding to each file
    % UK38_me - UK38_me indices corresponding to each file
    
    % Note: When calculating the UK37, UK38_et, and UK38_me indices, we
    % assume that the peaks used to calculate these indices are the final
    % six peaks in each file. If this is incorrect, you will need to
    % calculate the indices manually with the correct peaks.
    
    % Example: Output=getAlkenones;
    % Example: Output=getAlkenones('Files','GC27_2.csv','ShowGraphs',1,'SmoothParam',10);
    % Example: Output=getAlkenones('RT',[36.62, 37.25, 39.78, 40.05, 40.47, 40.65],'ShowGraphs',1);
    % Example: Output=getAlkenones('Files',{'GC27_1.csv','KNR197_1.csv'},'RT',{[36.62, 37.25, 39.78, 40.05, 40.47, 40.65];[33.63, 36.66, 37.19, 39.78, 40.05, 40.42, 40.62]},'ShowGraphs',1);
    ...
    % Calculate UK37 and UK38
    [UK37, UK38_et, UK38_me]=getUK(PeakAreas, PeakTimes);
    ...
    UK37=UK37(sort_index);
    UK38_et= UK38_et(sort_index);
    UK38_me= UK38_me(sort_index);
    ...
    Output.UK37=UK37;
    Output.UK38_et=UK38_et;
    Output.UK38_me=UK38_me;  
function [UK37, UK38_et, UK38_me]=getUK(Areas, PeakTimes)
    % This function calculates UK37, UK38_et, and UK38_me. It determines
    % which areas to use by finding the retention times closest to the
    % corresponding retention times in a model file.
    
    num_files=size(Areas,1);
    
    % Preallocate Arrays
    UK37    = zeros(num_files,1);
    UK38_et = zeros(num_files,1);
    UK38_me = zeros(num_files,1);
    ...
    for i=1:size(Areas,1)
        ...
        % Find the number of areas. Remove any empty areas
        tempAreas=Areas(i,:);
        tempTime=PeakTimes(i,:);
        tempAreas=tempAreas(tempTime~=0);
        tempAreas=tempAreas(~isnan(tempTime));
        
        % If there are more than six peaks, consider only the final six
        % peaks.
        n_peaks=length(tempAreas);
        if n_peaks>6
            tempAreas=tempAreas(n_peaks-5:end);
        end
        
        
        % UK37 = 37:2 / (37:2 + 37:3)
        try
            UK37(i)    = (tempAreas(Area_37_2))   ./(tempAreas(Area_37_2)    + tempAreas(Area_37_3));
        catch
            UK37(i) = NaN;
        end
        ...
        % UK38 = 38:2 / (38:2 + 38:3)
        try
            UK38_et(i) = (tempAreas(Area_38_2_et))./(tempAreas(Area_38_2_et) + tempAreas(Area_38_3_et));
        catch
            UK38_et(i) = NaN;
        end
        % UK38 = 38:2 / (38:2 + 38:3)
        try
            UK38_me(i) = (tempAreas(Area_38_2_me))./(tempAreas(Area_38_2_me) + tempAreas(Area_38_3_me));
        catch
            UK38_me(i) = NaN;
        end
    end
...
function [left, right, XGAUSS, YGAUSS, smoothY, gof, A, B]=findboundaries(X, Y, ind_peaks, file,IsShoulder,smoothParam)
    ...
    velocity=forward_derivative(X,smoothY);
    velocity=velocity./max(abs(velocity));
    smooth_velo=perfectSmoother(velocity,2*smoothParam);
